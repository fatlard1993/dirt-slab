...
public class SlabLeafLitterBlock extends PlantBlock implements OffsetableSlab {
	public static final MapCodec<SlabLeafLitterBlock> CODEC = createCodec(SlabLeafLitterBlock::new);
	...
	public SlabLeafLitterBlock(Settings settings) {
		super(settings);
	...
	protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
		int currentAmount = state.get(SEGMENT_AMOUNT);
		ItemStack stack = player.getMainHandStack();

		// Allow adding more leaf litter if player is holding leaf litter item
		if (stack.isOf(Items.LEAF_LITTER) && currentAmount < MAX_AMOUNT) {
			world.setBlockState(pos, state.with(SEGMENT_AMOUNT, currentAmount + 1), Block.NOTIFY_ALL);
