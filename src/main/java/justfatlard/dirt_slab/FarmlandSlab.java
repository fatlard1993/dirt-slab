...
public class FarmlandSlab extends SlicedTopSlab {
	...
	protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random){
		if(!state.canPlaceAt(world, pos)) SlabEffects.setToDirt(world, pos);
	...
	public void onLandedUpon(World world, BlockState state, BlockPos pos, Entity entity, double distance){
		if(!world.isClient() && world.random.nextFloat() < distance - 0.5F
			...
			&& (entity instanceof PlayerEntity || (world instanceof ServerWorld serverWorld && serverWorld.getGameRules().getValue(GameRules.DO_MOB_GRIEFING)))
